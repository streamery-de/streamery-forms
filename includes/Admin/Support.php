 ...
 * Class Support
 *
 * "Support Gutenform" links (Ko-fi, GitHub Sponsors, review) for the plugins
...
class Support
{
	 ...
	 * @var string
	 */
	const DISMISSED_META_KEY = 'gutenform_support_dismissed';
	...
	public static function get_links(): array
	{
		$links = array(
			...
			'githubSponsors' => '',
			'review'         => 'https://wordpress.org/support/plugin/gutenform-builder/reviews/#new-post',
		);
		 ...
		 */
		$links = apply_filters('gutenform/support_links', $links);
	...
	{
		if (plugin_basename(GUTENFORM_PLUGIN_FILE) !== $plugin_file) {
			return $meta;
		}

		$links = self::get_links();
		$items = array(
			'kofi'   => '&hearts; ' . esc_html__('Support on Ko-fi', 'gutenform-builder'),
			'review' => '&#9733; ' . esc_html__('Rate Gutenform', 'gutenform-builder'),
		);
	...
	public static function get_script_data(): array
	{
		...
		$dismissed = empty($links)
			|| !\Gutenform\Core\Capabilities::can_manage_settings()
			|| self::is_dismissed();
