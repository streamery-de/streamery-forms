/**
 * "Support Streamery Forms" data, localized by \StreameryForms\Admin\Support.
 *
...
const DISMISSED_EVENT = "streamery-forms:support-dismissed";

export function getSupportData(): SupportData {
	const data = (window as any).streameryForms?.support as Partial<SupportData> | undefined;
...
export async function dismissSupport(): Promise<void> {
	const data = (window as any).streameryForms?.support;
	if (data) {
		data.dismissed = true;
	}
